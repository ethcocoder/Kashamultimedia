...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Firebase is used only for the email/password admin entry. Its verified ID
    // token is exchanged into the same signed server session used by the app.
    // This fallback keeps admin tRPC calls safe if browsers block the session cookie.
    if (sessionToken && sessionToken.split(".").length === 3) {
      try {
        ...
        // Continue into the app-session flow; non-Firebase Bearer tokens are valid there.
      }
    }
