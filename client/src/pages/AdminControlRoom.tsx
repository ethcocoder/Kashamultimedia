...
import {
  ...
  CloudUpload, FileText, Image as ImageIcon, LayoutDashboard, LogOut, Mail,
...
const mutationError = (subject: string) => () => toast.error(`${subject} could not be saved.`, { description: "The Kasha desk could not complete that change. Please try again." });
...
const settingGroups = [
  { title: "Identity & hero", fields: [["siteName", "Site name"], ["brandLine", "Brand line"], ["heroEyebrow", "Hero eyebrow"], ["heroTitle", "Hero title"], ["heroAccent", "Hero accent"], ["heroIntro", "Hero intro", "long"], ["heroCtaLabel", "Hero CTA label"], ["heroImageUrl", "Hero image URL"], ["heroAsideTitle", "Hero aside title"], ["heroAsideBody", "Hero aside copy", "long"], ["heroFooterIndex", "Hero footer index"], ["heroFooterDescriptor", "Hero footer descriptor"], ["tickerItems", "Ticker items (separate with |)", "long"]] },
  { title: "About section", fields: [["aboutRailLabel", "Section rail label"], ["aboutEyebrow", "About eyebrow"], ["aboutTitle", "About title"], ["aboutAccent", "About accent"], ["aboutBody", "About body", "long"], ["aboutQuote", "About quotation", "long"], ["aboutImageUrl", "About image URL"], ["aboutCaptionLeft", "Image caption left"], ["aboutCaptionRight", "Image caption right"]] },
  ...
  { title: "Events & journal", fields: [["eventEyebrow", "Events eyebrow"], ["eventTitle", "Events title"], ["eventAccent", "Events accent"], ["eventBody", "Events body", "long"], ["eventCtaLabel", "Event CTA label"], ["eventImageUrl", "Event image URL"], ["eventImageLabel", "Event image label"], ["journalRailLabel", "Journal rail label"], ["journalEyebrow", "Journal eyebrow"], ["journalTitle", "Journal title"], ["journalAccent", "Journal accent"]] },
...
function ProgrammeEditor({ value, onChange, onClose, onSave, saving }: any) { return <EditorSheet title={value.id ? "Edit programme" : "Add programme"} description="Define the public programme card and its optional featured signal." onClose={onClose} onSave={onSave} saving={saving}><EditorFields value={value} onChange={onChange} fields={[["title", "Programme title"], ["subtitle", "Subtitle / translation"], ["tag", "Format tag"], ["sortOrder", "Display order", "number"], ["description", "Description", "long"], ["imageUrl", "Feature image URL"], ["featureTitle", "Feature title"], ["featureSubtitle", "Feature subtitle"], ["isPublished", "Publish now", "boolean"]]} /></EditorSheet>; }
...
function EventEditor({ value, onChange, onClose, onSave, saving }: any) { return <EditorSheet title={value.id ? "Edit event" : "Add event"} description="Place the next Kasha event into the public event feature." onClose={onClose} onSave={onSave} saving={saving}><EditorFields value={value} onChange={onChange} fields={[["title", "Event title"], ["ctaLabel", "CTA label"], ["ctaTarget", "CTA target"], ["sortOrder", "Display order", "number"], ["description", "Event description", "long"], ["imageUrl", "Event image URL"], ["isPublished", "Publish now", "boolean"]]} /></EditorSheet>; }
...
function MediaManager() {
  const utils = trpc.useUtils(); const list = trpc.admin.media.list.useQuery(); const upload = trpc.admin.media.upload.useMutation({ onSuccess: () => { utils.admin.media.list.invalidate(); toast.success("Image added to the Kasha media desk."); setFile(null); setAltText(""); setCategory("Editorial image"); }, onError: mutationError("Media upload") }); const remove = trpc.admin.media.remove.useMutation({ onSuccess: () => { utils.admin.media.list.invalidate(); toast.success("Media reference removed."); }, onError: mutationError("Media item") });
  const [file, setFile] = useState<File | null>(null); const [altText, setAltText] = useState(""); const [category, setCategory] = useState("Editorial image");
  const selectFile = (event: ChangeEvent<HTMLInputElement>) => setFile(event.target.files?.[0] ?? null);
  const submit = () => { if (!file || !altText.trim()) return toast.error("Choose an image and add useful alt text."); const reader = new FileReader(); reader.onload = () => upload.mutate({ fileName: file.name, altText, category, dataUrl: String(reader.result) }); reader.readAsDataURL(file); };
  return <div>
    <section className="desk-page-intro"><div><p className="eyebrow">Media desk / visual archive</p><h2>Give the story<br /><em>its image.</em></h2><p>Upload imagery once, copy the Kasha-hosted URL, then use it in site direction, programmes, or events.</p></div></section>
    <section className="desk-media-upload">
      <div><CloudUpload size={26} /><div><h3>Upload an image</h3><p>Images are stored securely and can be referenced across the public site.</p></div></div>
      <input type="file" accept="image/*" onChange={selectFile} />
      <Field label="Alt text" value={altText} onChange={setAltText} />
      <Field label="Category" value={category} onChange={setCategory} />
      <button className="desk-primary" type="button" onClick={submit} disabled={upload.isPending}>{upload.isPending ? "Uploading…" : "Add to media desk"} <ArrowUpRight size={16} /></button>
    </section>
